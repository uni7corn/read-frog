...
function scoreRepoFamiliarity(input) {
  ...
  const commitPoints =
    commitsInRepo === 0 ? 0 : commitsInRepo <= 5 ? 3 : commitsInRepo <= 20 ? 7 : 10

  const mergedPrs = input.prsInRepo.filter((pr) => pr.state === "merged").length
  const mergedPrPoints =
    mergedPrs === 0 ? 0 : mergedPrs === 1 ? 3 : mergedPrs <= 5 ? 6 : mergedPrs <= 15 ? 9 : 12

  const reviewsInRepo = toNumber(input.reviewsInRepo)
  const reviewPoints =
    reviewsInRepo === 0 ? 0 : reviewsInRepo <= 3 ? 3 : reviewsInRepo <= 10 ? 5 : 8
...
function scoreCommunityStanding(input) {
  ...
  const followerPoints =
    followers < 10 ? 1 : followers < 50 ? 3 : followers < 200 ? 5 : followers < 1000 ? 7 : 10

  const permissionBonus = POLICY.repoFamiliarityBonusPermissions.includes(
...
function scoreOSSInfluence(input) {
  ...
  const maxRepoPoints =
    maxStars === 0 ? 0 : maxStars <= 50 ? 3 : maxStars <= 500 ? 6 : maxStars <= 5000 ? 12 : 15
...
function scorePRTrackRecord(input) {
  ...
  const mergedPrs = input.prsInRepo.filter((pr) => pr.state === "merged").length
  const resolvedPrs = input.prsInRepo.filter(
...
export function getTrustBucket(total) {
  if (total >= 80) return TRUST_BUCKETS.HIGHLY_TRUSTED
  if (total >= 60) return TRUST_BUCKETS.TRUSTED
  if (total >= 30) return TRUST_BUCKETS.MODERATE
...
export function computeContributorScore(input) {
  ...
  const total =
    repoFamiliarity.score + communityStanding.score + ossInfluence.score + prTrackRecord.score
  const achievableMax = [
