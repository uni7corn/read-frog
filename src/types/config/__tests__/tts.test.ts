...
describe("tts config defaults", () => {
  ...
  it("only uses known Edge TTS voices in the default language voice map", () => {
    const languageVoices = createDefaultTTSLanguageVoices()
    const unknownDefaultVoices = Object.values(languageVoices).filter(
      (voice) => !isKnownEdgeTTSVoice(voice),
    )
  ...
  it("groups voice metadata by Microsoft Learn language labels", () => {
    ...
    expect(groupedItems.map((item) => item.voice).sort((a, b) => a.localeCompare(b))).toEqual(
      EDGE_TTS_VOICES,
    )
    expect(EDGE_TTS_VOICE_GROUPS).toContainEqual(
