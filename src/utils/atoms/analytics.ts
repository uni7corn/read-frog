...
export const analyticsEnabledAtom = atom(
  ...
  async (get, set, nextEnabled: boolean) => {
    ...
    try {
      await storageAdapter.set(ANALYTICS_ENABLED_STORAGE_KEY, nextEnabled, analyticsEnabledSchema)
    } catch (error) {
...
baseAnalyticsEnabledAtom.onMount = (setAtom) => {
  void storageAdapter
    .get(ANALYTICS_ENABLED_STORAGE_KEY, DEFAULT_ANALYTICS_ENABLED, analyticsEnabledSchema)
    .then(setAtom)

  const unwatch = storageAdapter.watch<boolean>(ANALYTICS_ENABLED_STORAGE_KEY, setAtom)
