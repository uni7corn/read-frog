...
function migrateDictionaryText(text: any): any {
  ...
  return text
    ...
    .replaceAll(
      "Keep Context short and directly tied to the selected text.",
      "Keep Paragraphs exactly as provided in the prompt.",
    )
    .replaceAll("- Context:", "- Paragraphs:")
    .replaceAll("- Context Translation:", "- Paragraphs Translation:")
    .replaceAll("\nContext: {{paragraphs}}", "\nParagraphs: {{paragraphs}}")
    .replaceAll(
      "根据给定的词语及其上下文，生成简洁的词典条目，匹配所需的输出对象。",
      "根据给定的词语及其周围段落，生成简洁的词典条目，匹配所需的输出对象。",
    )
...
function migrateDictionaryField(field: any): any {
  if (field?.id === "dictionary-context" || field?.id === "default-dictionary-context") {
    return {
      ...field,
      name:
        field.name === "语境" ? "段落内容" : field.name === "Context" ? "Paragraphs" : field.name,
      description:
        typeof field.description === "string"
          ? field.description
              .replaceAll(
                "The context in the prompt above, don't change it.",
                "The paragraphs from the prompt above. Do not rewrite them.",
              )
              .replaceAll(
                "The paragraphs in the prompt above, don't change them.",
                "The paragraphs from the prompt above. Do not rewrite them.",
              )
              .replaceAll(
                "上方提示词中的语境，请勿修改。",
                "使用上方提示词中的原始段落内容，不要改写。",
              )
              .replaceAll(
                "上方提示词中的段落内容，请勿修改。",
                "使用上方提示词中的原始段落内容，不要改写。",
